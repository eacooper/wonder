...
% Ths function visualized the patches used in the data structure for analyzing natural scene statistics of luminance
...
for scene = scenes
    ...
    load(['../Data/tsl/Scene_' sprintf('%03d',scene) '_1.mat'])
    
    %convert RGB to relative luminance
    imVorig = Data(:,:,2).*0.2989 + Data(:,:,3).*0.5870 + Data(:,:,4).*0.1140;
    
    
    subplot(2,2,subplotInd); hold on;
    imshow(imVorig./max(imVorig(:)));
    
    display(num2str(subplotInd));
    ...
    for x = patch_left_edges
        for y = patch_top_edges
            ...
            if length(Zpatch(~isnan(Zpatch)))/length(Zpatch(:)) > 0.95
                ...
                rectangle('Position',[x,y,patch_size_pix,patch_size_pix],'edgecolor',drawColors{randi(6)})
